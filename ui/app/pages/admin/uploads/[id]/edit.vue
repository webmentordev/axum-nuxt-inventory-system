<template>
    <section class="h-full w-full p-6">
        <div class="max-w-lg">
            <h1 class="text-xl font-bold text-white">Update File</h1>
            <p class="text-sm text-zinc-500 mt-1">Edit the file, its name, or its assignment.</p>

            <div v-if="loading" class="mt-6 text-sm text-zinc-500">Loading...</div>

            <form v-else @submit.prevent="handleSubmit" class="mt-6 flex flex-col gap-4" novalidate>
                <div>
                ...
                <div>
                    ...
                    <label
                        ...
                        <img v-if="previewUrl && fileType === 'image'" :src="previewUrl" alt="Preview"
                        ...
                        <span class="text-sm text-zinc-400">
                            {{ file ? file.name : (existingFileName || 'Click to select a file') }}
                        </span>
...
async function loadTargetOptions(value) {
...
async function loadUpload() {
    loading.value = true;
    try {
        const data = await authFetch(`/api/admin/uploads/${uploadId}`);
        if (!data) return;

        name.value = data.name || '';
        fileType.value = data.file_type || 'image';
        previewUrl.value = data.file_path || '';
        existingFileName.value = data.file_path ? data.file_path.split('/').pop() : '';

        let foundTarget = null;
        for (const key of Object.keys(targetKeyFromDetail)) {
            if (data[key]) {
                foundTarget = { type: targetKeyFromDetail[key], id: data[key].id, name: data[key].name };
                break;
            }
        }
        ...
        if (foundTarget) {
            ...
            targetType.value = foundTarget.type;
            targetOptionsMap.value[foundTarget.type] = [
                { label: foundTarget.name, value: foundTarget.id }
            ];
            targetId.value = foundTarget.id;
            loadTargetOptions(foundTarget.type);
        } else {
            assignMode.value = 'none';
        }
    } catch (e) {
        statusType.value = 'error';
        statusMessage.value = e.statusMessage || 'Failed to load upload.';
...
async function handleSubmit() {
    ...
    try {
        ...
        if (data) {
            ...
            statusMessage.value = 'File updated.';
            file.value = null;
            tempName.value = null;
            uploadProgress.value = 0;
        }
        ...
        statusType.value = 'error';
        statusMessage.value = e.statusMessage || 'Failed to update file.';
    } finally {
        saving.value = false;
        setTimeout(() => {
