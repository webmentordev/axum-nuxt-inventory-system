<template>
    <section class="h-full w-full p-6">
        <div class="max-w-lg">
            <h1 class="text-xl font-bold text-white">Create Category</h1>
            <p class="text-sm text-zinc-500 mt-1">Add a new category for your products.</p>
...
async function handleSubmit() {
    ...
    try {
        ...
        if (data) {
            ...
            statusMessage.value = 'Category created.';
            name.value = '';
            description.value = '';
        }
    } catch (e) {
        statusType.value = 'error';
        statusMessage.value = e.statusMessage || 'Failed to create category.';
    } finally {
        setTimeout(() => {
