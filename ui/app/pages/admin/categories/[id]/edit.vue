<template>
    <section class="h-full w-full p-6">
        <div class="max-w-lg">
            ...
            <p class="text-sm text-zinc-500 mt-1">Update this category's details.</p>
...
async function fetchCategory() {
    try {
        const data = await authFetch(`/api/admin/categories/${categoryId}`);
        if (data) {
        ...
        statusMessage.value = e.statusMessage || 'Failed to load category.';
...
async function handleSubmit() {
    ...
    try {
        ...
        if (data) {
            ...
            statusMessage.value = 'Category updated.';
        }
    } catch (e) {
        statusType.value = 'error';
