<template>
    <section class="h-full w-full p-6">
        <div class="max-w-lg">
            <h1 class="text-xl font-bold text-white">Edit brand</h1>
            <p class="text-sm text-zinc-500 mt-1">Update this brand's details.</p>
...
async function fetchBrand() {
    try {
        const data = await authFetch(`/api/admin/brands/${brandId}`);
        if (data) {
        ...
        statusMessage.value = e.statusMessage || 'Failed to load brand.';
...
async function handleSubmit() {
    ...
    try {
        ...
        if (data) {
            ...
            statusMessage.value = 'Brand updated.';
        }
    } catch (e) {
        statusType.value = 'error';
