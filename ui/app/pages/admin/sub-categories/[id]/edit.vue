<template>
    <section class="h-full w-full p-6">
        <div class="max-w-lg">
            <h1 class="text-xl font-bold text-white">Edit Sub-category</h1>
            <p class="text-sm text-zinc-500 mt-1">Update this sub-category's details.</p>
...
async function fetchSubCategory() {
    try {
        const data = await authFetch(`/api/admin/sub-categories/${subCategoryId}`);
        if (data) {
        ...
        statusMessage.value = e.statusMessage || 'Failed to load sub-category.';
...
async function handleSubmit() {
    ...
    try {
        ...
        if (data) {
            ...
            statusMessage.value = 'Sub-category updated.';
        }
    } catch (e) {
        statusType.value = 'error';
